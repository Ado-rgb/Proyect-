      ...
      for (const m of ev.messages) {
        ...
        try {
          ...
          if (citado && job) {
            ...
            if (!job._timer) {
              job._timer = setTimeout(() => delete pending[citado], 300000);
            }
          }
        } catch (e) {
          console.error("Error en respuestas citadas:", e);
        }
      }
    });
  }
...
async function downloadVideo(conn, job, asDocument, quoted) {
  ...
  await conn.sendMessage(chatId, {
    ...
    mimetype: "video/mp4",
    fileName: `${title}.mp4`,
    caption: asDocument ? undefined : "🎬 Video entregado por SYA TEAM BOT"
  }, { quoted });
  fs.unlinkSync(file);
}
