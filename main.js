...
case 'tovideo': {
  ...
  try {
    ...
    const conv = await axios.get(`https://api.neoxr.eu/api/webp2mp4?url=${encodeURIComponent(upload.data.url)}&apikey=GataDios`);
...
case 'visión2': {
    ...
    const query = args.join(" ");
    const apiUrl = `https://api.neoxr.eu/api/ai-anime?q=${encodeURIComponent(query)}&apikey=GataDios`;
...
case 'mediafire': {
    ...
    try {
        const apiUrl = `https://api.neoxr.eu/api/mediafire?url=${encodeURIComponent(mediafireUrl)}&apikey=GataDios`;
        const response = await fetch(apiUrl);
...
│🛠️ ${global.prefix}menufree
╰───────────────

╭─🧠 𝗜𝗔 - 𝗖𝗛𝗔𝗧
│🤖 ${global.prefix}chatgpt
│🌌 ${global.prefix}gemini
│🖼️ ${global.prefix}dalle
│👁️ ${global.prefix}vision
│🗣️ ${global.prefix}simi
╰───────────────

╭─⬇️ 𝗗𝗘𝗦𝗖𝗔𝗥𝗚𝗔𝗦
│🎵 ${global.prefix}play1/2/3
│🎶 ${global.prefix}ytmp3
│📹 ${global.prefix}ytmp4
│🎶 ${global.prefix}spotify
│🎥 ${global.prefix}tiktok / ig / fb
╰───────────────
...
╭─🎮 𝗝𝗨𝗘𝗚𝗢𝗦
│❤️ ${global.prefix}ship
│🧠 ${global.prefix}personalidad
│🎲 ${global.prefix}verdad / reto
╰───────────────
...
╭── 🛠️ 𝗘𝗡 𝗗𝗘𝗦𝗔𝗥𝗥𝗢𝗟𝗟𝗢
│💻 Nuevas funciones pronto... 
╰───────────────

👨‍💻 𝗕𝗢𝗧 𝗗𝗘𝗦𝗔𝗥𝗥𝗢𝗟𝗟𝗔𝗗𝗢 𝗣𝗢𝗥  
𝗦𝗬𝗔 𝗧𝗘𝗔𝗠 💖
`.trim();
    ...
    await sock.sendMessage2(
      msg.key.remoteJid,
      "🚫 *Error al mostrar el menú. Intenta otra vez.*",
      msg
    );
  }
  break;
...
case 'imagen': {
    ...
    const query = args.join(" ");
    const apiUrl = `https://api.neoxr.eu/api/goimg?q=${encodeURIComponent(query)}&apikey=GataDios`;
...
case 'info':
  try {
    ...
    const infoMessage = `╭─ *🤖 SYA TEAM BOT* ─╮
